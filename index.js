...
function _labelData(FileAttachment){return(
FileAttachment("labels.csv").csv({typed: true})
)}
...
Array.from(d3.rollup(data, ([d]) => d.value, d => +d.timestamp, d => d.name))
...
function bars(svg) {
  ...
  return ([timestamp, data], transition) => bar = bar
...
function labels(svg) {
  ...
  return ([timestamp, data], transition) => label = label
    .data(data.slice(0, n), d => d.name)
    .join(
...
function ticker(svg) {
  ...
  return ([timestamp], transition) => {
    transition.end().then(() => now.text(formatDate(timestamp)));
  };
}
)}

function _formatDate(labelData){return(
(timestamp) => labelData.find(m => m.timestamp == Math.floor(timestamp) + 1).label
)}
...
export default function define(runtime, observer) {
  ...
  const fileAttachments = new Map([
    ["labels.csv", {url: new URL("./files/labels.csv", import.meta.url), mimeType: "text/csv", toString}],
  ...
  main.variable(observer("formatDate")).define("formatDate", ["labelData"], _formatDate);
