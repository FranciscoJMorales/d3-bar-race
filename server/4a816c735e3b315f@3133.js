...
FileAttachment("results@5.csv").csv({typed: true})
)}

function _matches(FileAttachment){return(
FileAttachment("matches@1.csv").csv({typed: true})
)}
...
async function* _chart(replay,d3,width,height,bars,axis,labels,ticker,keyframes,duration,x,invalidation)
{
  ...
  const svg = d3.create("svg")
      ...
      .attr("width", width)
      .attr("height", height)
      .attr("style", "max-width: 1200px; height: auto;");
...
Array.from(d3.rollup(data, ([d]) => d.value, d => +d.matchday, d => d.name))
  .map(([matchday, data]) => [matchday, data])
...
function bars(svg) {
  ...
  return ([matchday, data], transition) => bar = bar
...
function labels(svg) {
  ...
  return ([matchday, data], transition) => label = label
    .data(data.slice(0, n), d => d.name)
    .join(
...
function _ticker(width,marginTop,barSize,n,formatDate,keyframes){return(
function ticker(svg) {
  ...
  return ([matchday], transition) => {
    transition.end().then(() => now.text(formatDate(matchday)));
  };
}
)}

function _formatDate(matches){return(
(matchday) => matches.find(m => m.id == Math.floor(matchday) + 1).match
)}
...
export default function define(runtime, observer) {
  ...
  const fileAttachments = new Map([
    ["matches@1.csv", {url: new URL("./files/513adc089869d382d96a8a67783ab37a05b5ac464cf2348e1a0c02249d610a9732d9756cb58b795ab0865f6ea0d538afb85142a0c65c02d8a2c1611e5afc2139.csv", import.meta.url), mimeType: "text/csv", toString}],
    ["results@5.csv", {url: new URL("./files/b972998e31b05d161f23e34966c0f8b51cb8132e6b22ada9e1682af110754983bed45408dc586438b67e5493c035623bcfda72bcc496dbf5a93dbe03a5fb97be.csv", import.meta.url), mimeType: "text/csv", toString}]
  ]);
  main.builtin("FileAttachment", runtime.fileAttachments(name => fileAttachments.get(name)));
  main.variable(observer()).define(["md"], _1);
  main.variable(observer("data")).define("data", ["FileAttachment"], _data);
  main.variable(observer("matches")).define("matches", ["FileAttachment"], _matches);
  ...
  main.variable(observer("ticker")).define("ticker", ["width","marginTop","barSize","n","formatDate","keyframes"], _ticker);
  main.variable(observer("formatDate")).define("formatDate", ["matches"], _formatDate);
